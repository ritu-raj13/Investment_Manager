...
class PortfolioSettings(db.Model):
    ...
    mc_threshold_rank_100 = db.Column(db.Float, nullable=True)
    mc_threshold_rank_250 = db.Column(db.Float, nullable=True)
    mc_threshold_rank_500 = db.Column(db.Float, nullable=True)
    mc_thresholds_updated_at = db.Column(db.DateTime, nullable=True)
...
def _market_cap_tier_from_details(details):
    """Map company MC (Cr) to Large/Mid/Small/Micro using saved portfolio thresholds."""
    if not details or details.get('market_cap_cr') is None:
        return None
    settings = PortfolioSettings.query.first()
    if not settings:
        return None
    return classify_market_cap_tier(
        details['market_cap_cr'],
...
def refresh_stock_day_change():
    ...
    return jsonify({
        'message': (
            ...
            f'{skipped} skipped (non-Indian symbols).'
        ),
        'updated': updated,
        'failed': failed,
        'skipped': skipped,
...
def get_portfolio_summary():
    ...
    try:
        ...
        realized_breakdown = []
        for symbol, holding in holdings.items():
            amount = round(holding.get('realized_pnl', 0) or 0, 2)
            if amount != 0:
                realized_breakdown.append({
                    'symbol': _display_symbol(symbol),
                    'name': holding.get('name'),
                    'amount': amount,
                    'closed': holding['quantity'] <= 0,
                })
        realized_breakdown.sort(key=lambda x: x['amount'], reverse=True)

        unrealized_breakdown = [
            {
                'symbol': _display_symbol(h['symbol']),
                'name': h['name'],
        ...
        unrealized_breakdown.sort(key=lambda x: x['amount'], reverse=True)
        ...
        return jsonify({
            ...
            'unrealized_breakdown': unrealized_breakdown,
        })
    except Exception as e:
...
def update_portfolio_settings():
    ...
    if 'mc_threshold_rank_100' in data:
        v = data['mc_threshold_rank_100']
        settings.mc_threshold_rank_100 = float(v) if v is not None and v != '' else None
    if 'mc_threshold_rank_250' in data:
        v = data['mc_threshold_rank_250']
        settings.mc_threshold_rank_250 = float(v) if v is not None and v != '' else None
    if 'mc_threshold_rank_500' in data:
        v = data['mc_threshold_rank_500']
        settings.mc_threshold_rank_500 = float(v) if v is not None and v != '' else None
...
@app.route('/api/portfolio/settings/refresh-mc-thresholds', methods=['POST'])
@api_login_required
def refresh_mc_thresholds():
    """Fetch Screener market-cap screen and save rank 100/250/500 cutoffs (Rs. Cr)."""
    try:
        triple = fetch_market_cap_rank_thresholds()
        if not triple:
            return jsonify({
                'error': 'Could not load enough companies from Screener (need 500). Try again later.',
            }), 502
        t100, t250, t500 = triple
        settings = PortfolioSettings.query.first()
        if not settings:
            settings = PortfolioSettings()
            db.session.add(settings)
        settings.mc_threshold_rank_100 = t100
        settings.mc_threshold_rank_250 = t250
        settings.mc_threshold_rank_500 = t500
        settings.mc_thresholds_updated_at = datetime.now(timezone.utc)
        settings.updated_at = datetime.now(timezone.utc)
        db.session.commit()
        return jsonify({
            'message': 'Market cap thresholds updated from Screener.',
            'mc_threshold_rank_100': t100,
            'mc_threshold_rank_250': t250,
            'mc_threshold_rank_500': t500,
            'mc_thresholds_updated_at': settings.mc_thresholds_updated_at.isoformat(),
        })
    except Exception as e:
        print(f"[ERROR] refresh_mc_thresholds: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500


def _sqlite_file_path_from_uri():
...
def _run_sqlite_migrations_after_create_all():
    """db.create_all() does not ALTER existing tables; keep SQLite in sync with models."""
    db_path = _sqlite_file_path_from_uri()
    if not db_path or not os.path.isfile(db_path):
        return
    try:
        from migrations.db_migrator import DatabaseMigrator
...
def _build_recommendation_context():
    """Shared data prep for recommendation endpoints."""
    stocks = Stock.query.all()
    transactions = PortfolioTransaction.query.all()
    holdings_dict = calculate_holdings(transactions)
    ...
    for symbol, holding in holdings_dict.items():
        ...
        stock = stocks_map.get(normalized_symbol)
        current_price = stock.current_price if stock else None
        current_value = (holding['quantity'] * current_price) if current_price else 0.0
        holdings_list.append({
            'symbol': symbol,
            'name': stock.name if stock else '',
            'sector': stock.sector if stock else None,
    ...
    settings = PortfolioSettings.query.first()
    total_target_amount = (
        settings.projected_portfolio_amount
        if settings and settings.projected_portfolio_amount > 0
...
@app.route('/api/recommendations/price-zones', methods=['GET'])
@api_login_required
def get_price_zone_recommendations():
    """Price-zone-only recommendations screen payload."""
    try:
        stocks, holdings_dict, _, _, _ = _build_recommendation_context()
        action_items = _build_price_zone_action_items(stocks, holdings_dict)
        ...
        print(f"ERROR in get_price_zone_recommendations: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500


@app.route('/api/recommendations/rebalancing', methods=['GET'])
@api_login_required
def get_rebalancing_recommendations():
    """Rebalancing-only recommendations payload."""
    try:
        stocks, _, holdings_list, settings, total_target_amount = _build_recommendation_context()
        ...
        return jsonify(rebalancing)
    except Exception as e:
        print(f"ERROR in get_rebalancing_recommendations: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
...
def get_recommendations_dashboard():
    ...
    Kept temporarily while frontend migrates to split endpoints.
    """
    try:
        stocks, holdings_dict, holdings_list, settings, total_target_amount = _build_recommendation_context()
        from utils import get_rebalancing_suggestions
